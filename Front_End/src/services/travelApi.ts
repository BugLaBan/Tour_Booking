...
const normalizeParticipationStatus = (
  value?: string,
): AdminOrder["participationStatus"] =>
...
const mapSchedule = (schedule: TourScheduleApi): TourSchedule => ({
  ...
  availableSlots: parseNumber(
    schedule.availableSlots ??
      parseNumber(schedule.availableSeats) - parseNumber(schedule.bookedSeats),
  ),
...
const mapOrderItem = (item: OrderItemApi): AdminOrderItem => ({
  id: String(
    item.id ?? `${item.tourId ?? "tour"}-${item.scheduleId ?? "schedule"}`,
  ),
...
const parseDuration = (value: string) => {
  ...
  return {
    ...
    nights: parseNumber(
      matches[1] ?? Math.max(parseNumber(matches[0] ?? 1) - 1, 0),
    ),
  };
};
...
export const toAdminTourPayload = (formData: AdminTourFormState) => {
  ...
  const returnDate = departureDate
    ? addDays(formData.startDate, Math.max(duration.days - 1, 0))
    : "";
...
export const toAdminTourFormState = (
  tour: TourDetailApi,
  categoryId = "",
): AdminTourFormState => ({
  title: tour.tourName || "",
  ...
  remainingSlots: parseNumber(
    tour.schedules?.[0]?.availableSlots ?? tour.schedules?.[0]?.availableSeats,
  ),
  childRemainingSlots: 0,
  infantRemainingSlots: 0,
  duration: formatDuration(tour.durationDays, tour.durationNights),
...
export const fetchPublicToursPage = async (
...
export const fetchTourDetail = async (slug: string) => {
  const response = await unwrap<TourDetailApi>(
    apiClient.get(`/tours/${encodeURIComponent(slug)}`),
  );
...
export const calculateTourPrice = async (payload: {
  ...
  const response = await unwrap<PriceQuoteApi>(
    apiClient.post("/tours/calculate-price", payload),
  );
  return {
...
export const updateCategoryApi = async (
  id: string,
  category: Partial<FrontendCategory>,
) =>
  unwrap(
...
export const deleteCategoryApi = async (id: string) =>
  unwrap(apiClient.delete(`/category/${id}`));
...
export const createAdminTour = async (
  payload: ReturnType<typeof toAdminTourPayload>,
) => unwrap<number>(apiClient.post("/admin/tours", payload));

export const updateAdminTour = async (
  id: string,
  payload: ReturnType<typeof toAdminTourPayload>,
) => unwrap(apiClient.put(`/admin/tours/${id}`, payload));

export const deleteAdminTour = async (id: string) =>
  unwrap(apiClient.delete(`/admin/tours/${id}`));
...
export const updateOrderStatusApi = async (
  ...
  unwrap(
    apiClient.patch(`/order/${id}/status`, null, {
      params: { status: status.toUpperCase() },
    }),
  );

export const updateOrderPaymentStatusApi = async (
  id: string,
  status: AdminOrder["paymentStatus"],
) =>
  unwrap(
    apiClient.patch(`/order/${id}/payment-status`, null, {
      params: { status: status.toUpperCase() },
    }),
  );
...
export const deleteUserApi = async (id: string) =>
  unwrap(apiClient.delete(`/user/delete/${id}`));
...
const buildNewsContent = (tour: Tour) => {
  const description =
    tour.description ||
    `Hanh trinh ${tour.title} dang thu hut nhieu du khach trong thoi gian gan day.`;
