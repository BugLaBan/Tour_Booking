...
const OrderCheckIn = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | AdminOrder["checkInStatus"] | "ready" | "waiting_payment"
  >("ready");
  ...
  const [checkInDrafts, setCheckInDrafts] = useState<
    Record<string, typeof emptyDraft>
  >({});
  ...
  const loadOrders = async (
    ...
    try {
      ...
      if (preferredId) {
        setSelectedOrder(
          data.find((order) => order.id === preferredId) ?? data[0] ?? null,
        );
      } else {
        setSelectedOrder(
          (current) =>
      ...
      setError(
        getApiErrorMessage(cause, "Khong the tai man hinh check-in luc nay."),
      );
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    setCheckInDrafts(
      Object.fromEntries(
        selectedOrder.items.map((item) => [item.id, buildCheckInDraft(item)]),
      ),
    );
  }, [selectedOrder]);
  ...
  const handleSubmitCheckIn = async (item: AdminOrderItem) => {
    ...
    try {
      ...
      setError(
        getApiErrorMessage(cause, "Khong the cap nhat check-in cho muc nay."),
      );
    } finally {
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-500">
            Staff co the loc don san sang, xem khach da xac nhan tham gia hay
            chua, va cap nhat check-in theo tung tour trong don.
          </p>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      {order.items[0]?.name || "Tour dang cap nhat"}
                    </p>
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  <div className="rounded-2xl bg-gray-50 px-4 py-3 text-sm">
                    ...
                    <p className="mt-2 font-bold text-gray-900">
                      {participationLabel[selectedOrder.participationStatus]}
                    </p>
                    {selectedOrder.participationConfirmedAt !==
                      "Dang cap nhat" && (
                      <p className="mt-1 text-xs text-gray-500">
                        {selectedOrder.participationConfirmedAt}
                      </p>
                  ...
                  <div className="rounded-2xl bg-gray-50 px-4 py-3 text-sm">
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Da thu {selectedOrder.paidAmount.toLocaleString()}d
                    </p>
                    <p className="mt-1 text-xs text-gray-500">
                      Moc toi thieu{" "}
                      {selectedOrder.requiredDepositAmount.toLocaleString()}d
                    </p>
                  </div>
                  <div className="rounded-2xl bg-gray-50 px-4 py-3 text-sm">
                    <p className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-500">
                    ...
                    <p
                      className={`mt-2 font-bold ${selectedOrder.outstandingAmount > 0 ? "text-amber-700" : "text-emerald-700"}`}
                    >
                      {selectedOrder.outstandingAmount.toLocaleString()}d
                    </p>
                    <p className="mt-1 text-xs text-gray-500">
                      Han: {selectedOrder.balanceDueDate}
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    >
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                          ...
                          <p className="mt-2 text-sm text-gray-500">
                            Dat {totalBooked} khach | Da den{" "}
                            {item.checkedInAdultQuantity +
                              item.checkedInChildQuantity +
                              item.checkedInInfantQuantity}{" "}
                            | Vang{" "}
                            {item.noShowAdultQuantity +
                              item.noShowChildQuantity +
                              item.noShowInfantQuantity}
                          </p>
                        </div>
                      ...
                      <div className="mt-4 grid gap-3 md:grid-cols-3">
                        <label className="text-xs font-medium text-gray-600">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              handleCheckInDraftChange(
                                item.id,
                                "checkedInAdultQuantity",
                                event.target.value,
                              )
                            }
                      ...
                      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                        ...
                        <div className="rounded-2xl bg-white px-4 py-3 text-sm text-gray-600">
                          <p>
                            Tam tinh: da den {totalCheckedIn} / {totalBooked}
                          </p>
                          <p>
                            Tam tinh: vang {totalNoShow} / {totalBooked}
                          </p>
                          <button
                            type="button"
