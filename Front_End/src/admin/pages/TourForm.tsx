...
const TourForm = () => {
      ...
      try {
        ...
        if (isEdit && id) {
          ...
          const matchingCategory = categoryData.find(
            (item) =>
              item.slug === detail.categorySlug ||
              item.name === detail.categoryName,
          );
          setFormData(toAdminTourFormState(detail, matchingCategory?.id ?? ""));
        }
      } catch (err) {
        if (active)
          setError(getApiErrorMessage(err, "Khong the tai du lieu tour."));
      } finally {
  ...
  const addDay = () => {
    setFormData((prev) => ({
      ...
      itinerary: [
        ...prev.itinerary,
        { day: prev.itinerary.length + 1, title: "", content: "" },
      ],
    }));
  };
  ...
  const removeDay = (index: number) => {
    setFormData((prev) => ({
      ...
      itinerary: prev.itinerary
        ...
        .map((day, itemIndex) => ({ ...day, day: itemIndex + 1 })),
    }));
  };
  ...
  const categoryOptions = useMemo(
    () =>
      categories.filter(
        (item) => item.status === "active" || item.id === formData.categoryId,
      ),
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-2xl font-bold text-gray-800">
          {isEdit ? "Chinh sua tour" : "Tao tour"}
        </h2>
      </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <input
            ...
            value={formData.slug}
            onChange={handleInputChange}
            placeholder={slugify(formData.title)}
            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-4">
            ...
            <input
              ...
              onChange={handleInputChange}
              className="w-full h-fit px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-3">
            ...
            <input
              ...
              onChange={handleInputChange}
              placeholder="Vi du: 3 Ngay 2 Dem"
              className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none"
            />
            <input
              type="date"
        ...
        <textarea
          ...
          onChange={handleInputChange}
          rows={6}
          placeholder="Thong tin tour"
          className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none resize-none"
        />
                ...
                <div className="bg-gray-50 p-4 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 flex-grow">
                    ...
                    <input
                      ...
                      placeholder={`Ngay ${day.day}`}
                      className="flex-grow bg-transparent font-bold text-gray-800 outline-none"
                    />
                  </div>
                  <button
                    type="button"
        ...
        <div className="pt-8 border-t flex flex-col items-center gap-4">
          <button
            type="submit"
            disabled={isSaving}
            className="w-full max-w-md bg-primary text-white py-3 rounded-xl font-bold hover:bg-primary/90 transition-all shadow-lg disabled:opacity-70"
          >
