      ...
      try {
        ...
        if (active) {
          ...
          setSelectedScheduleId(
            data.selectedScheduleId || data.schedules?.[0]?.id || "",
          );
  ...
  useEffect(() => {
    ...
    const quotePrice = async () => {
      ...
      try {
        ...
        if (active) {
          setPriceError(
            "Khong the tinh gia tu dong. He thong dang hien thi gia tam tinh.",
          );
          setPriceQuote(fallbackQuote(tour, quantities));
        }
      } finally {
    ...
    <div className="bg-gray-50 min-h-screen pb-20">
      <div className="relative h-64 md:h-80 flex items-center overflow-hidden">
        ...
        <div className="container relative z-10 text-white">
          <h1 className="text-3xl md:text-5xl font-black uppercase mb-4 max-w-4xl">
            {tour.title}
          </h1>
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-8">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
              ...
              <p className="text-gray-600 leading-relaxed">
                {tour.description || "Dang cap nhat mo ta tour."}
              </p>
            </div>
              ...
              <div className="flex flex-col gap-6">
                {(tour.itinerary ?? []).length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-gray-200 px-6 py-8 text-center text-sm text-gray-500">
                    <p>
                      Tour nay chua co lich trinh chi tiet theo tung ngay tu
                      backend.
                    </p>
                  ...
                  (tour.itinerary ?? []).map((item) => (
                    <div
                      key={item.day}
                      className="relative pl-12 pb-8 border-l-2 border-dashed border-primary/30 last:border-0 last:pb-0"
                    >
                ...
                <div className="flex gap-4 mb-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-[10px] text-gray-400">
                      {tour.reviewsCount} luot danh gia
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-3 text-sm">
                  ...
                  <div className="flex items-center gap-3 text-gray-600">
                    ...
                    <span>
                      Ngay Khoi Hanh:{" "}
                      <span className="font-bold text-gray-900">
                        {selectedSchedule?.departureDate || tour.startDate}
                      </span>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {selectedSchedule ? (
                    <p className="text-xs text-gray-500">
                      Ve: {selectedSchedule.returnDate} | Trang thai:{" "}
                      {selectedSchedule.status}
                    </p>
                  ) : (
                    <p className="text-xs text-amber-600">
                      Tour nay chua co lich khoi hanh phu hop.
                    </p>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all"
                      >
                        <Minus size={14} />
                      </button>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-gray-400">
                        {(tour.childPrice ?? 0).toLocaleString()}d
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-gray-400">
                        {(tour.infantPrice ?? 0).toLocaleString()}d
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                        type="button"
                        onClick={() => handleQuantityChange("infants", -1)}
                        className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all"
                      >
                        <Minus size={14} />
                      </button>
                ...
                <div className="rounded-2xl bg-gray-50 p-4 text-sm text-gray-600">
                  ...
                  {isPricing && (
                    <p className="mt-1 text-xs text-gray-500">
                      Dang tinh gia theo lich khoi hanh...
                    </p>
                  )}
                  {priceError && (
                    <p className="mt-1 text-xs text-amber-600">{priceError}</p>
                  )}
                </div>
