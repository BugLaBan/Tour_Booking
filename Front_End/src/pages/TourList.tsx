...
const parseSortFromSearch = (search: string) =>
  new URLSearchParams(search).get("sort") || "newest";
const parsePageFromSearch = (search: string) =>
...
const TourList = () => {
  ...
  const regionTitle = regionSlug
    ? (regionNames[regionSlug] ?? regionSlug)
    : null;
    ...
    const loadTours = async () => {
      ...
      try {
        ...
        const totalPassengers =
          filters.adultCount + filters.childCount + filters.infantCount;
  ...
  const destinationOptions = useMemo(
    () =>
      [
        ...new Set(
          tours.map((tour) => tour.location).filter(Boolean) as string[],
        ),
      ].sort((left, right) => left.localeCompare(right)),
    [tours],
  );
  ...
  const syncQuery = (
    ...
    if (nextFilters.adultCount > 0)
      params.set("adultCount", String(nextFilters.adultCount));
    if (nextFilters.childCount > 0)
      params.set("childCount", String(nextFilters.childCount));
    if (nextFilters.infantCount > 0)
      params.set("infantCount", String(nextFilters.infantCount));
    if (nextFilters.priceRange !== "all")
      params.set("priceRange", nextFilters.priceRange);
