      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="group">
            <Link
              to={mainNews.path}
              className="block overflow-hidden rounded-3xl relative aspect-video mb-6"
            >
              <SafeImage
                src={mainNews.image}
                alt={mainNews.title}
            ...
            <Link
              to={mainNews.path}
              className="text-primary font-bold flex items-center gap-1 hover:text-secondary transition-colors"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {sideNews.map((item) => (
              <div key={item.id} className="flex flex-col gap-3 group">
                <Link
                  to={item.path}
                  className="block overflow-hidden rounded-2xl relative aspect-[4/3]"
                >
                  <SafeImage
                    src={item.image}
                    alt={item.title}
