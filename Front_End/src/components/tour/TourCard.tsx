...
const TourCard: React.FC<TourCardProps> = ({ tour, variant = "default" }) => {
  ...
  const slotsLabel =
    tour.remainingSlots > 0 ? String(tour.remainingSlots) : "Lien he";
  const detailPath = `/tour/${encodeURIComponent(tour.slug || tour.id)}`;

  return (
    <div
      className={`bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group border border-gray-100 ${isFlashSale ? "border-primary/20" : ""}`}
    >
            ...
            <div className="flex text-yellow-400">
              {[...Array(5)].map((_, index) => (
                <Star
                  ...
                  size={12}
                  fill={
                    index < Math.floor(tour.rating) ? "currentColor" : "none"
                  }
                />
              ))}
            </div>
          ...
          <div className="flex flex-col">
            {tour.originalPrice ? (
              <span className="text-xs text-gray-400 line-through">
                {tour.originalPrice.toLocaleString()}d
              </span>
            ) : null}
